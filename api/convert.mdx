...
<Note>
  The conversion logic is implemented in the ICE server backend (web_server.py:118-125). The Flask API routes the request to the appropriate ICE method:
  
  - `temperatura` → `convertirTemperatura()`
  - `longitud` → `convertirLongitud()`
  - `peso` → `convertirPeso()`
  - `velocidad` → `convertirVelocidad()`
</Note>
